...
const NavDropdown: React.FC<Props> = ({ label, items, active, variant = 'nav', menuMinWidth = 210 }) => {
  ...
  const triggerStyle: React.CSSProperties = variant === 'control'
    ? {
        ...
        border: `1px solid ${open ? 'rgba(43,187,146,0.5)' : 'rgba(255,255,255,0.16)'}`,
          ...
          {items.map(item => (
            <button
              ...
              style={{
                ...
                background: item.active
                  ? 'linear-gradient(92deg, rgba(43,187,146,0.30), rgba(139,12,246,0.30))'
                  : 'transparent',
            ...
            >
              <span style={{
                ...
                background: item.active ? '#2BBB92' : 'rgba(255,255,255,0.25)',
              }} />
              {item.label}
            </button>
          ))}
