...
if (fs.existsSync(filePath)) {
  ...
  rmRows.forEach(row => {
    ...
    const jornadaCol = rowKeys.find(k => k.toLowerCase() === 'campo personalizado (jornada)') || 'Campo personalizado (Jornada)';
    const timeCol = rowKeys.find(k => k.toLowerCase() === 'time') || 'time';
    ...
    const timeVal = String(row[timeCol] || '').replace(/;;/g, '').trim().toUpperCase();
    
    const comboKey = `Jornada (customField): ${jornadaVal} | timeCol: ${timeVal} | Squad (customTime): ${customTimeVal}`;
    comboCounts[comboKey] = (comboCounts[comboKey] || 0) + 1;
  });
  
  console.log('\nCombinations of Jornada (customField), timeCol, and Squad in RM project:');
